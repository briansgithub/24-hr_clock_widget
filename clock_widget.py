...
class ClockWidget:
    ...
    def on_enter(self, event):
        if self.hover_timer is not None:
            ...
            self.hover_timer = None
        if self.is_transparent:
            self.hover_timer = self.root.after(750, self.make_solid) # Sets the hover-over delay
    ...
    def fetch_sun_times(self):
        def _fetch():
            lat, lon = None, None
            try:
                # Lightest weight ping for location
            ...
            try:
                ...
                print("Could not comte sunrise/sunset:", e)
            finally:
    ...
    def update_fitbit_data(self):
        def _task():
            try:
                start, end = self.fitbit.get_main_sleep_times()
                ...
                print(f"Fitbit background update failed: {e}")
       
        threading.Thread(target=_task, daemon=True).start()

    def show_initial(self):
        self.set_transparency(True)
        self.update_fitbit_data() # Initial fetch
        self.draw_clock()
        self.root.deiconify()
    ...
    def create_tray_image(self, now):
        ...
        # PIL angles: 0 is 3 o'clock, clockwise.
        # 18:00 is bottom (90 deg), 06:00 is top (270 deg)
        ...
        hand_angle = math.radians((current_hour + 6) * 15) # +6h shift to align 18:00 with 90deg (bottom)
    ...
    def update_clock(self):
        ...
        # update tray icon if minute changed
        ...
        # Calculate milliseconds until the next 10-second mark
        seconds_to_next_mark = 10 - (now.second % 10)
        ms_to_next_mark = seconds_to_next_mark * 1000 - int(now.microsecond / 1000)
        # Schedule next update at the next 10-second mark (with a small buffer)
    ...
    def draw_clock(self):
        ...
        # Draw Sleep Region (Fitbit data)
        if self.show_sleep.get() and self.sleep_hour is not None and self.wake_hour is not None:
            sleep_duration = self.wake_hour - self.sleep_hour
            if sleep_duration < 0:
                sleep_duration += 24
               
            sleep_start_angle = (18 - self.sleep_hour) * 15
            sleep_extent = - (sleep_duration * 15)
           
            # Draw sleep arc slightly inset
        ...
        for h_tick in range(24):
            ...
            # Use appropriate logic to check if it's night time for this tick mark
...
''' Hex Colors Options:
Cool & modern (high contrast): #00A8FF (bright sky blue)
Subtle but clear: #FF9F1C (soft orange)
Night-friendly / less harsh: #9B5DE5 (muted violet)
Minimalist / understated: #2EC4B6 (teal)
High visibility (especially on dark faces): #FFD60A (warm yellow)
'''
